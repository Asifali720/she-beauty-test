...
import type { ClaimItems } from '@/types/claim'
...
const ClaimCollapsibleTable = ({ head, children, open, claimId }: Props) => {
  const theme = useTheme()

  //use Query for getting claim items
  const { data, error, isLoading, isError } = useQuery({
    queryKey: ['claimItems', claimId],
    queryFn: () => AdminClaimService.getClaimsItem(claimId),
              ...
              <Table stickyHeader aria-label='purchases'>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    data?.claimItems?.map((item: ClaimItems, index: number) => {
...
export default ClaimCollapsibleTable

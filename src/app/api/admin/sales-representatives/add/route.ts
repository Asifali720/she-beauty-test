...
export async function POST(request: NextRequest) {
  try {
    ...
    const newSalesRepresentatives = new SalesRepresentativesModel({
      name,
      photo: photoUrl,
      phone_no,
      email,
    ...
    const savedSalesRepresentatives = await newSalesRepresentatives.save()

    return NextResponse.json({
      success: true,
      message: 'Sales Representative created successfully',
      salesRepresentative: savedSalesRepresentatives
    })
  } catch (error: any) {

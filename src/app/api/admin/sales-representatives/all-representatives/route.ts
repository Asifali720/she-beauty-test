...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalCount = await SalesRepresentativesModel.countDocuments({
      status: { $ne: 'deleted' }
    ...
    const salesRepresentatives = await SalesRepresentativesModel.find({ status: { $ne: 'deleted' } }, {}, query)

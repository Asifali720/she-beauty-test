...
export function formatTime(apiResponseTimestamp: any) {
  if (!apiResponseTimestamp) return 'N/A'

  const date = new Date(apiResponseTimestamp)
  const day = String(date.getDate()).padStart(2, '0')
  const month = String(date.getMonth() + 1).padStart(2, '0') // Months are 0-indexed
  const year = date.getFullYear()

  const formattedDate = `${day}-${month}-${year}` // Rearrange the order

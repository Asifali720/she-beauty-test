...
export async function GET(request: NextRequest) {
  ...
  try {
    const totalCount = await AdjustmentsModal.countDocuments() // Get total number of documents
    ...
    if (startDate && endDate) {
      adjustments = await AdjustmentsModal.find(
        {
          createdAt: {
      ...
      adjustments = await AdjustmentsModal.find({}, {}, query).populate('distributor')
    }

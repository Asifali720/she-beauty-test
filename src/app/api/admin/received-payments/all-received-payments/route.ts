...
export async function GET(request: NextRequest) {
  ...
  try {
    const totalCount = await ReceivedPaymentsModel.countDocuments() // Get total number of documents
    ...
    if (startDate && endDate) {
      receivedPayments = await ReceivedPaymentsModel.find(
        {
          createdAt: {
      ...
      receivedPayments = await ReceivedPaymentsModel.find({}, {}, query).populate('distributor')
    }

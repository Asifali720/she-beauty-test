...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const products = await ProductModel.find({ status: { $ne: 'deleted' } }, {}, query).populate('raw_items.raw_item')

    const productsWithTotalQty = await Promise.all(
      products.map(async product => {
        const totalQty = await calculateTotalQuantity(product)

        return { ...product?.toObject(), totalQty } // Add totalQty to the product object
      })
    )
...
const calculateTotalQuantity = async (product: any) => {
  ...
  product.raw_items.forEach((item: any) => {
    const possibleQty = Math.floor(item?.raw_item?.quantity / item?.quantity) // Round down to nearest integer
